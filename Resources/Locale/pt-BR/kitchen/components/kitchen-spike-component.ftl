comp-kitchen-spike-deny-collect = { CAPITALIZE(THE($this)) } já tem algo ai, colete sua carne primero!
comp-kitchen-spike-deny-butcher = { CAPITALIZE(THE($victim)) } não pode ser cortado no(a) { THE($this) }.
comp-kitchen-spike-deny-butcher-knife = { CAPITALIZE(THE($victim)) } não pode ser cortado(a) no(a) { THE($this) }, você precisa corta-lo usando uma faca.
comp-kitchen-spike-deny-not-dead = { CAPITALIZE(THE($victim)) } não pode ser cortado(a). { CAPITALIZE(SUBJECT($victim)) } { CONJUGATE-BE($victim) } não está morto(a)!
