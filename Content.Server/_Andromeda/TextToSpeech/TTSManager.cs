...
public sealed class TTSManager : ITTSManager
{
    ...
    public void Initialize()
    {
        ...
        _cfg.OnValueChanged(AndromedaCCVars.TTSApiUrl, x => _apiUrl = x, true);
        _cfg.OnValueChanged(AndromedaCCVars.TTSApiToken, x => _apiToken = x, true);
        _cfg.OnValueChanged(AndromedaCCVars.TTSApiTimeout, x => _timeout = x, true);
    }

    public async Task<byte[]?> ConvertTextToSpeechStandard(int voice, string text)
    {
        _sawmill.Warning("Convertendo TTS");
